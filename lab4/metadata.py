...
def figMetaData(file_path):
    ...
    print(f"{dir(img_doc)}\n")
    decimalLong = convertGPScoordinate(
        img_doc.gps_longitude, img_doc.gps_longitude_ref)
    decimalLat = convertGPScoordinate(
        img_doc.gps_latitude, img_doc.gps_latitude_ref)
    print(f"Latitude: {decimalLat} Longitude: {decimalLong}")
    url = f"http://www.google.com/maps/place/{decimalLat},{decimalLong}"
...
if __name__ == "__main__":
    ...
    parser = optparse.OptionParser(
        "Usage: python skriptalab4.py -f hotel.jpeg")
